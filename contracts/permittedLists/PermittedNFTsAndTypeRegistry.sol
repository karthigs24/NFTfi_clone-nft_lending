...
contract PermittedNFTsAndTypeRegistry is Ownable, IPermittedNFTs {
    ...
    modifier onlyOwnerOrAirdropFactory(string memory _nftType) {
        if (
            ...
            require(
                hub.getContract(ContractKeys.AIRDROP_FACTORY) == _msgSender(),
                "caller is not AirdropFactory"
            );
        } else {
     ...
     */
    function getNFTPermit(address _nftContract)
        external
        view
        override
        returns (bytes32)
     ...
     * NFTs.
     */
    function setNftType(string memory _nftType, address _nftWrapper)
        external
        onlyOwner
    {
     ...
     * with NFTs.
     */
    function _setNftTypes(
        string[] memory _nftTypes,
        address[] memory _nftWrappers
    ) internal {
        require(
            _nftTypes.length == _nftWrappers.length,
            "setNftTypes function information arity mismatch"
        );
     ...
     */
    function _setNFTPermit(address _nftContract, string memory _nftType)
        internal
    {
        ...
        if (nftTypeKey != 0) {
            require(
                getNftTypeWrapper(nftTypeKey) != address(0),
                "NFT type not registered"
            );
        }

        require(
            nftPermits[_nftContract] !=
     ...
     * - != "" means "enable permit with the given NFT Type"
     */
    function _setNFTPermits(
        address[] memory _nftContracts,
        string[] memory _nftTypes
    ) internal {
        require(
            _nftContracts.length == _nftTypes.length,
            "setNFTPermits function information arity mismatch"
        );
